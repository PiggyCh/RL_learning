...
def training(env,arg):
    ...
    for training_time in range(arg.iteration_time):
        ...
        while not done:
            ...
            if not done:
                a_new = epsilon_policy(s, arg.epsilon, Q_table)
                Q_table[s, a] += arg.alpha * (r + arg.gamma * Q_table[s_new, a_new] - Q_table[s, a])
                s, a = s_new, a_new
            else:
                Q_table[s, a] += arg.alpha * (r - Q_table[s, a])
        if training_time>15000 and training_time %1 ==0:
